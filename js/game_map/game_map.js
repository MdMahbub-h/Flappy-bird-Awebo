...
init(params) {
	...
	this.button_play = new CustomButton(this.scene, loading_vars.W / 2, loading_vars['H'] * 0.8, () => {
		this.handler_play();
...
create_score_panel() {
	...
	let back = new Phaser.GameObjects.Image(this.scene, 0, 0, 'common1', 'panel8');
	back.setScale(0.5);
	this.score_panel.add(back);
	back.setInteractive({useHandCursor: true});
	back.on('pointerdown', () => {
		let pt = game_data['utils'].toGlobal(this.score_panel, new Phaser.Geom.Point(0, 0));
		game_data['utils'].show_tip({'pt': pt, 'scene_id': 'game_tip', 'item_id': 'tip', 'phrase_id': 'record', 'values': []});
	}, this)
	this.score_txt = new Phaser.GameObjects.Text(this.scene, 0, -10, game_data['user_data']['global_score'], {fontFamily:"font2", fontSize: 35, color:'#a86233', stroke: '#ffe1a1', strokeThickness: 3});
	this.score_txt.setOrigin(0.5, 1);
	this.score_panel.add(this.score_txt);
	this.score_txt.setText(this.score);

	let score_icon = new Phaser.GameObjects.Image(this.scene, 0, this.score_txt.y + 30, 'common1', 'rating_ico');
...
new_score() {
	let pt = game_data['utils'].toGlobal(this.score_panel, new Phaser.Geom.Point(0, 0));
	game_data['utils'].show_tip({'pt': pt, 'scene_id': 'game_tip', 'item_id': 'tip', 'phrase_id': 'new_record', 'values': []});
},
...
create_lang_button() {
	...
	this.button_lang2 = new CustomButton(this.scene, 580, 50, this.handler_lang, 'common1', 'btn_lang', 'btn_lang', 'btn_lang',  this);
...
create_remove_ad_button() {
	...
	this.button_ad2 = new CustomButton(this.scene, 180, 50, this.handler_remove_ad, 'common1', 'btn_rem_ad', 'btn_rem_ad', 'btn_rem_ad',  this);
...
create_sound_buttons() {
	...
	this.button_music_on = new CustomButton(this.scene, 380, 50, this.handler_music, 'common1', 'btn_music_on', 'btn_music_on', 'btn_music_on',  this);
	this.button_music.add(this.button_music_on);
	this.button_music_off = new CustomButton(this.scene, 380, 50, this.handler_music, 'common1', 'btn_music_off', 'btn_music_off', 'btn_music_off', this);
	...
	this.button_sound_on = new CustomButton(this.scene, 480, 50, this.handler_sound, 'common1', 'btn_sound_on', 'btn_sound_on', 'btn_sound_on', this);
	this.button_sound.add(this.button_sound_on);
	this.button_sound_off = new CustomButton(this.scene, 480, 50, this.handler_sound, 'common1', 'btn_sound_off', 'btn_sound_off', 'btn_sound_off', this);
...
show_map(obj = {}) {
	if (obj['complete']) {
		if (!game_data['user_data']['tutorial']['map_snitch']) {
			...
			game_request.request({'update_tutorial': true, 'tutorial_id': 'map_snitch'}, ()=>{});
		}
	}
	this.update_money();
},
...
create_assets() {
	...
	this.money_icon = new Phaser.GameObjects.Image(this.scene, 55, 55, 'common1', 'star');
	game_data['money_holder'].add(this.money_icon);

	this.money_txt = new Phaser.GameObjects.Text(this.scene, 55, 55, game_data['user_data']['money'], {fontFamily:"font2", fontSize: 35, color:'#a86233', stroke: '#ffe1a1', strokeThickness: 3});
	this.money_txt.setOrigin(0.5);
	game_data['money_holder'].add(this.money_txt);
	this.game_icon_anim();

	this.money_icon.setInteractive({ useHandCursor: true });
	this.money_icon.on('pointerup', () => {
		this.snitch_tip();
	}, this)
},
...
snitch_tip(tutorial_tip = false) {
	...
	game_data['utils'].show_tip({'pt': pt, 'scene_id': 'game_tip', 'item_id': 'tip', 'phrase_id': 'snitch', 'values': []});
},
...
game_icon_anim() {
	if (this.scene) {
		...
		for (let i = 0; i < game_data['money_holder'].lights.length; i++) {
			...
			game_data['scene'].tweens.add({targets: img, scale: 1.8, duration: 500, yoyo: true, repeat: 0, delay: 500 + i * 900 , ease: 'Sine.easeOut'});
