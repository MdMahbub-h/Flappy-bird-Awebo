var GamePlay = new Phaser.Class({
	...
	init(params) {
        ...
        this.bird = this.scene.physics.add.sprite(loading_vars['W'] / 2 - 130, loading_vars['H'] / 2, 'common1', bird_name);
        this.add(this.bird);
        this.resetBirdOptions();

        this.score = 0;
        this.topScore = localStorage.getItem(gameOptions.localStorageName) == null ? 0 : localStorage.getItem(gameOptions.localStorageName);
        this.scoreText = this.scene.add.text(loading_vars.W / 2, 120, '', {...game_data['styles']['title'], fontFamily:"font2", fontSize: 55});
        ...
        this.scene.physics.add.overlap(this.bird, this.starGroup, function(player, coin){
            if (coin.active) {
    ...
    addInfo(){
        ...
        let res = game_data['utils'].generate_string({'scene_id': 'game_windows', 'item_id': 'level_finished', 'phrase_id': this.mode, 'values': [], 'base_size': 80});
        let temp = new Phaser.GameObjects.Text(this.scene, loading_vars['W'] / 2, 450 - 20 - 250, res['text'], {fontFamily:"font1", fontSize: res['size']})
        temp.setOrigin(0.5);
        this.add(temp);
        this.mode_txt = temp;
        this.infoGroup.add(this.mode_txt);

        res = game_data['utils'].generate_string({'scene_id': 'game_play', 'item_id': 'game_play', 'phrase_id': '1', 'values': [], 'base_size': 80});
        temp = new Phaser.GameObjects.Text(this.scene, loading_vars['W'] / 2, 450 - 20 - 150, res['text'], {fontFamily:"font1", fontSize: res['size']})
        temp.setOrigin(0.5);
        this.add(temp);
        this.start_txt = temp;
        this.infoGroup.add(this.start_txt);

        res = game_data['utils'].generate_string({'scene_id': 'game_tip', 'item_id': 'tip', 'phrase_id': 'snitch', 'values': [], 'base_size': 45});
        temp = new Phaser.GameObjects.Text(this.scene, loading_vars['W'] / 2, 830, res['text'], {fontFamily:"font1", fontSize: res['size'], color: '#fff', stroke: '#000', strokeThickness: 3, align: 'center', wordWrap: {'width': 620}})
        temp.setOrigin(0.5);
        this.add(temp);
        this.snitch_txt = temp;
        this.infoGroup.add(this.snitch_txt);

        res = game_data['utils'].generate_string({'scene_id': 'game_tip', 'item_id': 'tip', 'phrase_id': 'snitch2', 'values': [], 'base_size': 20});
        temp = new Phaser.GameObjects.Text(this.scene, loading_vars['W'] / 2, 830 + 80, res['text'], {fontFamily:"font1", fontSize: res['size'], color: '#fff', stroke: '#000', strokeThickness: 3, align: 'center', wordWrap: {'width': 620}})
        temp.setOrigin(0.5);
        this.add(temp);
        this.snitch_txt2 = temp;
        this.infoGroup.add(this.snitch_txt2);

        this.tap = this.scene.add.sprite(loading_vars['W'] / 2 - 130, 450 - 20 + 250, "common1", "tap");
        this.add(this.tap);
    ...
    start_level({ mode }) {
        ...
        res = game_data['utils'].generate_string({'scene_id': 'game_tip', 'item_id': 'tip', 'phrase_id': 'snitch', 'values': [], 'base_size': 80});
        this.snitch_txt.setText(res['text']);

        res = game_data['utils'].generate_string({'scene_id': 'game_tip', 'item_id': 'tip', 'phrase_id': 'snitch2', 'values': [], 'base_size': 80});
        this.snitch_txt2.setText(res['text']);

        if (this.mode === 'easy') {
            this.mode_txt.setColor(game_data['styles']['easy_text']['color']);
            this.mode_txt.setStroke(game_data['styles']['easy_text']['stroke'], game_data['styles']['easy_text']['strokeThickness']);

            this.start_txt.setColor(game_data['styles']['easy_text']['color']);
            this.start_txt.setStroke(game_data['styles']['easy_text']['stroke'], game_data['styles']['easy_text']['strokeThickness']);
        }
        else if (this.mode === 'normal') {
            this.mode_txt.setColor(game_data['styles']['normal_text']['color']);
            this.mode_txt.setStroke(game_data['styles']['normal_text']['stroke'], game_data['styles']['normal_text']['strokeThickness']);

            this.start_txt.setColor(game_data['styles']['normal_text']['color']);
            this.start_txt.setStroke(game_data['styles']['normal_text']['stroke'], game_data['styles']['normal_text']['strokeThickness']);
        }
        else if (this.mode === 'hard') {
            this.mode_txt.setColor(game_data['styles']['hard_text']['color']);
            this.mode_txt.setStroke(game_data['styles']['hard_text']['stroke'], game_data['styles']['hard_text']['strokeThickness']);

            this.start_txt.setColor(game_data['styles']['hard_text']['color']);
            this.start_txt.setStroke(game_data['styles']['hard_text']['stroke'], game_data['styles']['hard_text']['strokeThickness']);
        }
        else if (this.mode === 'crazy') {
            this.mode_txt.setColor(game_data['styles']['crazy_text']['color']);
            this.mode_txt.setStroke(game_data['styles']['crazy_text']['stroke'], game_data['styles']['crazy_text']['strokeThickness']);

            this.start_txt.setColor(game_data['styles']['crazy_text']['color']);
            this.start_txt.setStroke(game_data['styles']['crazy_text']['stroke'], game_data['styles']['crazy_text']['strokeThickness']);
        }
    },
    ...
    level_failed() {
        if (allow_rewarded_ads && this.currentStageAdsWatched < game_data['allowed_trials']) {
            ...
            this.level_active = false;
            this.emitter.emit('EVENT', {'event': 'show_window', 'window_id': 'level_failed', 'score': this.score, 'currentStage': this.currentStage, 'currentStageAdsWatched': this.currentStageAdsWatched, 'mode': this.mode});
        }
        else {
    ...
    updateScore(inc){
        if (inc) game_data['audio_manager'].sound_event({'play': true, 'sound_name': 'level_complete4'});
        this.score += inc;
    ...
    placePipes(addScore, allowStar = false){
        ...
        let pipeHolePosition = Phaser.Math.Between(gameOptions.minPipeHeight + pipeHoleHeight / 2, loading_vars['H'] - gameOptions.minPipeHeight - pipeHoleHeight / 2);
        ...
        let texture = Phaser.Utils.Array.GetRandom(['pipe1', 'pipe2', 'pipe3', 'pipe4']);
    ...
    placeStar(pool1, pool2, star) {
        ...
        if (start_point > end_point) [start_point, end_point] = [end_point, start_point];
    ...
    flap(){
        if (this.flap_count === 0) {
            ...
            let coef = 1;
            if (this.mode in gameOptions.birdSpeedCoef) coef = gameOptions.birdSpeedCoef[this.mode];
    ...
    update(){
        if (this.level_active) {
            this.scene.physics.world.collide(this.bird, this.pipeGroup, () => {
                game_data['audio_manager'].sound_event({'play': true, 'sound_name': 'pipe_hit'});
                this.die();
            }, null, this);
