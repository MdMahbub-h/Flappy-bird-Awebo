let Shop = new Phaser.Class({
    ...
    create_assets(params) {
		...
		this.cont_current_knife = new Phaser.GameObjects.Container(this.scene, loading_vars.W / 2, 435);
		...
		this.title = new Phaser.GameObjects.Text(this.scene, 0, -250, 'Harry', {...game_data['styles']['title'], fontFamily:"font2", fontSize: 82});
		...
		this.btn_left = new CustomButton(this.scene, loading_vars.W * 0.1, this.cont_current_knife.y, this.handler_left, 'common1', 'btn_play1', 'btn_play1', 'btn_play1', this, null, null, 1);
		this.add(this.btn_left);
		this.btn_left.setScale(-0.7, 0.7);

		this.btn_right = new CustomButton(this.scene, loading_vars.W * 0.9, this.cont_current_knife.y, this.handler_right, 'common1', 'btn_play1', 'btn_play1', 'btn_play1', this, null, null, 1);
		...
		this.btn_money = new CustomButton(this.scene, loading_vars.W / 2, loading_vars.H * 0.8, () => { 
			this.handler_buy_money_item();
		 }, 'common1', 'btn_green', 'btn_green', 'btn_green', this, null, null, 1);
		this.add(this.btn_money);
		let txt = new Phaser.GameObjects.Text(this.scene, 45, -3, '', {...game_data['styles']['light_text'], fontFamily:"font2", fontSize: 24});
		this.btn_money.add(txt);
		txt.setOrigin(1, 0.5);
		res = game_data['utils'].generate_string({'scene_id': 'shop', 'item_id': 'shop', 'phrase_id': '1', 'values': [], 'base_size': 24});
		txt.setText(res['text']);
		this.btn_money.txt = txt;
		this.btn_money.ico = new Phaser.GameObjects.Image(this.scene, 0, -3, 'common1', 'star').setScale(0.4);
		this.btn_money.add(this.btn_money.ico);
		this.btn_money.ico.x = this.btn_money.txt.x + this.btn_money.txt.displayWidth / 2;

		txt = new Phaser.GameObjects.Text(this.scene, 0, -3, '', {...game_data['styles']['purp_text'], fontFamily:"font2", fontSize: 24});
		this.btn_ad.add(txt);
		txt.setOrigin(0.5);
		res = game_data['utils'].generate_string({'scene_id': 'shop', 'item_id': 'shop', 'phrase_id': '2', 'values': [], 'base_size': 24});
		txt.setText(res['text']);
		this.btn_ad.txt = txt;
	...
	handler_buy_money_item() {
		let current_item = this.get_current_item();
		game_request.request({'buy_item': true, 'type': 'money', 'id': current_item.id}, res => {
			if (res['success']) {
				let pt_end = game_data['game_map'].get_money_pt();
                let pt_start = game_data['utils'].toGlobal(this.btn_money, new Phaser.Geom.Point(this.btn_money.ico.x, this.btn_money.ico.y));
				...
				let pt = game_data['utils'].toGlobal(game_data['money_holder'], new Phaser.Geom.Point(40, 65));
			...
			for (let i = 0; i < 3; i++) {
				for (let j = 0; j < 3; j++) {
					if (money_iter < this.money_items.length) {
						this.create_item({ i, j, page, info: this.money_items[money_iter] });
						money_iter++;
					}
				}
			}
	...
	create_item(params) {
		...
		back.on('pointerup', () => {
			if (this.current_id !== id) {
				...
				if (game_data['user_data']['money_resourses'].includes(item.id) || game_data['user_data']['ad_resourses'].includes(item.id)) {
		...
		if (game_data['user_data']['money_resourses'].includes(id) || game_data['user_data']['ad_resourses'].includes(id)) {
			back.setFrame('back1');
	...
	update_btn_text() {
		...
		if (this.btn_money && this.btn_money.txt) {
			...
			let res = game_data['utils'].generate_string({'scene_id': 'shop', 'item_id': 'shop', 'phrase_id': '1', 'values': [price], 'base_size': 24});
			this.btn_money.txt.setText(res['text']);
			this.btn_money.txt.setFontSize(res['size']);
			if (this.btn_money.ico) {
				this.btn_money.ico.x = this.btn_money.txt.x + this.btn_money.txt.displayWidth / 2 - 40;
			}
	...
	update_buttons({ only_switch = false }) {
		...
		if (game_data['user_data']['money_resourses'].includes(id) ||
		...
		let res = game_data['utils'].generate_string({'scene_id': 'shop', 'item_id': 'shop', 'phrase_id': id, 'values': [], 'base_size': 50});
	...
	update_language() {
		...
		let item = this.get_current_item();
		res = game_data['utils'].generate_string({'scene_id': 'shop', 'item_id': 'shop', 'phrase_id': '1', 'values': [item.price], 'base_size': 24});
		this.btn_money.txt.setText(res['text']);
		this.btn_money.txt.setFontSize(res['size']);
		this.btn_money.ico.x = this.btn_money.txt.x + this.btn_money.txt.displayWidth / 2 - 40;
	}
